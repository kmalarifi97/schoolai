# Build brief — Concept 02: The Cavendish Experiment — weighing the Earth

**Arabic:** تجربة كافندش — وزن الأرض
...
## What to build
- One short concept-spark video: `cavendish_full_voiced_subs.mp4`
- Beats are in `cavendish.json` → `beats[]` (id / text / visual). English narration, Arabic subs burned in, pure #000000 void.
- `[Hold Ns in silence]` markers in a beat's visual mean exactly that: hold the frame, no narration.
- Pipeline + conventions: `manim/CLAUDE.md` (Standard pipeline section). Slug = `cavendish`.
- Produce the video here.

## Hand-off contract
Deliver `cavendish_full_voiced_subs.mp4` in this directory. Tone is non-negotiable: silence is content, kitchen-table-warm, concept before equations, no worked problems, no hype.
