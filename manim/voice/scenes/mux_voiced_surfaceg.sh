#!/usr/bin/env bash
# For each surfaceg beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_surfaceg"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_surfaceg_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 9); do
  CLASS="SurfacegS1B${i}"
  VIDEO="scenes/media/videos/surfaceg_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_surfaceg_en/s1_b${i}.mp3"
  OUT="voiced_surfaceg/voiced_surfaceg_b${i}.mp4"
