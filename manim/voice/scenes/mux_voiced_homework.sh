#!/usr/bin/env bash
# For each homework beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_homework"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_homework_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 25); do
  CLASS="HomeworkS1B${i}"
  VIDEO="scenes/media/videos/homework_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_homework_en/s1_b${i}.mp3"
  OUT="voiced_homework/voiced_homework_b${i}.mp4"
