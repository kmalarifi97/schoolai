#!/usr/bin/env bash
# For each weighearth beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_weighearth"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_weighearth_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 8); do
  CLASS="WeighearthS1B${i}"
  VIDEO="scenes/media/videos/weighearth_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_weighearth_en/s1_b${i}.mp3"
  OUT="voiced_weighearth/voiced_weighearth_b${i}.mp4"
  ...
  # record per-beat audio duration for subtitle timing
