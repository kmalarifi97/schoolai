...
OUT_DIR="$PROJ_VOICE/voiced_paperclip"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_paperclip_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 30); do
  CLASS="PaperclipS1B${i}"
  VIDEO="scenes/media/videos/paperclip_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_paperclip_en/s1_b${i}.mp3"
  OUT="voiced_paperclip/voiced_paperclip_b${i}.mp4"
  [ ! -f "$PROJ_VOICE/$VIDEO" ] && { echo "MISSING video: $VIDEO"; exit 1; }
  [ ! -f "$PROJ_VOICE/$AUDIO" ] && { echo "MISSING audio: $AUDIO"; exit 1; }

  V_DUR=$(probe "$VIDEO"); A_DUR=$(probe "$AUDIO")
