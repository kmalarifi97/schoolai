#!/usr/bin/env bash
# For each newton beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_newton"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_newton_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 11); do
  CLASS="NewtonS1B${i}"
  VIDEO="scenes/media/videos/newton_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_newton_en/s1_b${i}.mp3"
  OUT="voiced_newton/voiced_newton_b${i}.mp4"
