#!/usr/bin/env bash
# For each orbital beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_orbital"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_orbital_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 10); do
  CLASS="OrbitalS1B${i}"
  VIDEO="scenes/media/videos/orbital_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_orbital_en/s1_b${i}.mp3"
  OUT="voiced_orbital/voiced_orbital_b${i}.mp4"
