#!/usr/bin/env bash
# For each kepler3 beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_kepler3"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_kepler3_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 14); do
  CLASS="Kepler3S1B${i}"
  VIDEO="scenes/media/videos/kepler3_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_kepler3_en/s1_b${i}.mp3"
  OUT="voiced_kepler3/voiced_kepler3_b${i}.mp4"
