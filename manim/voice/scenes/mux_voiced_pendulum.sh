#!/usr/bin/env bash
# For each pendulum beat: pad video/audio to max length and mux.
set -euo pipefail
export LC_ALL=C
cd "$(dirname "$0")"
PROJ_VOICE="$(cd .. && pwd)"

OUT_DIR="$PROJ_VOICE/voiced_pendulum"
mkdir -p "$OUT_DIR"
TAIL=0.30
...
DUR_LIST="$PROJ_VOICE/durations_pendulum_en.txt"
: > "$DUR_LIST"

for i in $(seq 1 29); do
  CLASS="PendulumS1B${i}"
  VIDEO="scenes/media/videos/pendulum_b${i}/720p30/${CLASS}.mp4"
  AUDIO="audio_pendulum_en/s1_b${i}.mp3"
  OUT="voiced_pendulum/voiced_pendulum_b${i}.mp4"
